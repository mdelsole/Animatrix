...
def sumFilter(imgIn, radius):
    ...
    if np.size(radius)== 4:
        imgOut = scp.convolve2d(imgIn,np.ones((radius(2)+radius(4)+1,radius(1)+radius(3)+1,radius(2)+radius(4)+1,radius(1)+radius(3)+1)), mode='same')
    if np.size(radius)== 1:
        imgOut = scp.convolve2d(imgIn,np.ones((2*radius+1,2*radius+1)), 'same')
